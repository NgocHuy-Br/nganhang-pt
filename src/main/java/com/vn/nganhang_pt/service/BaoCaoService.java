...
public class BaoCaoService {
    ...
    public List<GiaoDich> saoKeGiaoDich(String soTK, LocalDate tuNgay, LocalDate denNgay,
        ...
        try (Connection conn = DriverManager.getConnection(connectionString, username, password);
                CallableStatement stmt = conn.prepareCall("{CALL SP_XemSKKHACHHANG(?, ?, ?)}")) {
    ...
    public byte[] xuatPDFSaoKe(String soTK, LocalDate tuNgay, LocalDate denNgay,
            String tenServer, String username, String password,
            String chuTK, String chiNhanh, String nguoiXuat) throws Exception {

        // Lấy dữ liệu giao dịch
        List<GiaoDich> danhSach = saoKeGiaoDich(soTK, tuNgay, denNgay, tenServer, username, password);

        // Tính số dư đầu và cuối kỳ
        java.math.BigDecimal soDuDau = danhSach.isEmpty() ? java.math.BigDecimal.ZERO : danhSach.get(0).getSoDuDau();
        ...
        Paragraph soDuDauNgay = new Paragraph(
                "Số dư đầu ngày " + pdfConfig.formatDate(tuNgay) + ": " + pdfConfig.formatCurrency(soDuDau))
                .setFont(fontBold)
                .setFontSize(PdfConfig.FONT_SIZE_INFO)
                .setBold()
                .setMarginTop(2)
        ...
        Paragraph soDuCuoiNgay = new Paragraph(
                "Số dư đến cuối ngày " + pdfConfig.formatDate(denNgay) + ": " + pdfConfig.formatCurrency(soDuCuoi))
                .setFont(fontBold)
                .setFontSize(PdfConfig.FONT_SIZE_INFO)
                .setBold()
                .setMarginTop(2)
        ...
        for (GiaoDich gd : sortedList) {
            ...
            table.addCell(new Cell()
                    .add(new Paragraph(String.valueOf(stt++)).setFont(font).setFontSize(PdfConfig.FONT_SIZE_TABLE_BODY))
            ...
            String ngayGD = gd.getNgay() != null
                    ...
                    : "";
            table.addCell(
                    new Cell().add(new Paragraph(ngayGD).setFont(font).setFontSize(PdfConfig.FONT_SIZE_TABLE_BODY))
                            .setPadding(5));

            // Loại giao dịch
            table.addCell(new Cell()
                    .add(new Paragraph(gd.getLoaiGiaoDich()).setFont(font).setFontSize(PdfConfig.FONT_SIZE_TABLE_BODY))
                    .setPadding(5));
    ...
    public List<Map<String, Object>> lietKeTaiKhoanMoi(LocalDate tuNgay, LocalDate denNgay,
        ...
        // NGANHANG role sẽ xem tất cả chi nhánh, CHINHANH chỉ xem chi nhánh mình
        int tatCaChiNhanh = "NGANHANG".equals(role) ? 1 : 0;

        try (Connection conn = DriverManager.getConnection(connectionString, username, password);
                CallableStatement stmt = conn.prepareCall("{CALL SP_LietKeTaiKhoanMoiMo(?, ?, ?)}")) {

            stmt.setDate(1, Date.valueOf(tuNgay));
            stmt.setDate(2, Date.valueOf(denNgay));
            stmt.setInt(3, tatCaChiNhanh);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    row.put("TENCHINHANH", rs.getString("ChiNhanh"));
                    danhSach.add(row);
                }
            }
        } catch (SQLException e) {
            System.err.println("[ERROR] Lỗi liệt kê tài khoản mới: " + e.getMessage());
    ...
    public byte[] xuatPDFTaiKhoanMoi(LocalDate tuNgay, LocalDate denNgay, String role, String maCN,
        ...
        for (Map<String, Object> row : danhSach) {
            ...
            table.addCell(new Cell().add(new Paragraph(soDu).setFont(font).setFontSize(PdfConfig.FONT_SIZE_TABLE_BODY))
                    .setTextAlignment(TextAlignment.RIGHT).setPadding(5));

            // Ngày mở
            String ngayMo = row.get("NGAYDK") != null ? sdf.format(row.get("NGAYDK")) : "";
            table.addCell(
                    new Cell().add(new Paragraph(ngayMo).setFont(font).setFontSize(PdfConfig.FONT_SIZE_TABLE_BODY))
                            .setTextAlignment(TextAlignment.CENTER).setPadding(5));
